...
def pv_LAA_centerlines(inputfile, seedsfile, outfile, pvends=1):
    ...
    (other side) PVs"""
    """
    inputfile=args.meshfile; seedsfile=outseedsfile; outfile; pvends=
    """
...
def clip_veins_sections_and_LAA(inputfile, sufixfile, clspacing, maxslope, skippointsfactor, highslope, bumpcriterion):
    ...
    """
    inputfile=args.meshfile
    sufixfile=outfile
    clspacing=args.clspacing
    maxslope=args.maxslope
    skippointsfactor=args.skippointsfactor
    highslope=args.highslope
    bumpcriterion=args.bumpcriterion
    """
    ...
    for k in range(1, 6):
        ...
        # writevtp(cl, sufixfile + 'clbranch' + str(k) + '_cellthreshold.vtp')
        cl = vmtkcenterlinemerge(cl)
        # writevtp(cl, sufixfile + 'clbranch' + str(k) + '_cellthreshold.merge.vtp')
        ...
        cl = vmtkcenterlineresampling(cl, clspacing)
        cl = vmtkcenterlineattributes(cl)
        # writevtp(cl, sufixfile + 'clbranch' + str(k) + '_cellthreshold.merge.attributes.vtp')
        #-----------------------------------------------------------------------
        # Tiral clips
