...
def pointthreshold(polydata, arrayname, start=0, end=1, alloff=0):
    # pointthreshold( polydata=mesh, arrayname='hole', start=0, end=0)
    """
    polydata = mesh
    arrayname = "hole"
    start=1; end=1
...
png_visualise(surfer.GetOutput())


png_visualise(polydata)
"""
...
def extractboundaryedge(polydata):
    ...
    # edge.BoundaryEdgesOn()  # Extract boundary edges
    # edge.FeatureEdgesOff()  # Disable feature edge extraction
    # edge.NonManifoldEdgesOn()  # Disable non-manifold edge extraction
    # edge.ManifoldEdgesOff()  # Disable manifold edge extraction
    # edge.Update()
...
def transfer_cell_array(ref, target, arrayname, targetarrayname):
    """Transfer scalar array using closest point approximation"""
    locator = vtk.vtkCellLocator()
    locator.SetDataSet(ref)
    locator.BuildLocator()

    refarray = ref.GetCellData().GetArray(arrayname)  # get array from reference

    numberofcells = target.GetNumberOfCells()
    newarray = vtk.vtkDoubleArray()
    newarray.SetName(targetarrayname)
    newarray.SetNumberOfTuples(numberofcells )
    target.GetCellData().AddArray(newarray)

    # go through each point of target surface, determine closest point on surface, copy value
    for i in range(target.GetNumberOfCells()):
        cell = target.GetCell(i)
        closestpoint_id = locator.FindClosestPoint(cell)
        value = refarray.GetValue(closestpoint_id)
        newarray.SetValue(i, value)
    return target
...
def transfer_all_scalar_arrays(m1, m2):
    ...
    # for i in range(m1.GetCellData().GetNumberOfArrays()):
    #     print('Transferring scalar array: {}'.format(m1.GetCellData().GetArray(i).GetName()))
    #     transfer_cell_array(m1, m2, m1.GetCellData().GetArray(i).GetName(), m1.GetCellData().GetArray(i).GetName())
...
def extract_LA_contours(m_open, filename, save=False, edge_length=15):
    """
    filename=args.meshfile; save=args.save_conts
    """
    ...
    """
    writevtk(m_open, sufixfile+'extractboundaryedge(m_open).vtk')
    """
    save=True
...
def png_which_min2(vec, target):
    return int(np.argmin( np.abs(vec-target) ))
...
def get_rspv_segments_ids(cont_rspv, locator_open, v1l, v1d, v1r, propn_rspv_s1, propn_rspv_s2, propn_rspv_s3):
    ...
    rspv_s1 = rspv_ids[0:pos_v1d]
    rspv_s2 = rspv_ids[pos_v1d: pos_v1r]
    rspv_s3 = rspv_ids[pos_v1r: rspv_ids.size]
...
def get_ripv_segments_ids(cont_ripv, locator_open, v2l, v2r, v2u, propn_ripv_s1, propn_ripv_s2, propn_ripv_s3):
    ...
    ripv_s1 = ripv_ids[0:pos_v2r]
    ripv_s2 = ripv_ids[pos_v2r: pos_v2r]
    ripv_s3 = ripv_ids[pos_v2u: ripv_ids.size]
...
def get_lipv_segments_ids(cont_lipv, locator_open, v3r, v3u, v3l, propn_lipv_s1, propn_lipv_s2, propn_lipv_s3):
    ...
    pos_v3r = png_which_min2(lipv_cont_ids, v3r)
    lipv_ids = np.append(lipv_cont_ids[pos_v3r:lipv_cont_ids.size], lipv_cont_ids[0:pos_v3r])
    pos_v3u = png_which_min2(lipv_ids, v3u)
    pos_v3l = png_which_min2(lipv_ids, v3l)
    ...
    lipv_s1 = lipv_ids[0:pos_v3u]
    lipv_s2 = lipv_ids[pos_v3u: pos_v3l]
    lipv_s3 = lipv_ids[pos_v3l: lipv_ids.size]
...
def get_lspv_segments_ids(cont_lspv, locator_open, v4r, v4u, v4d, propn_lspv_s1, propn_lspv_s2, propn_lspv_s3):
    ...
    pos_v4r = png_which_min2(lspv_cont_ids, v4r)
    lspv_ids = np.append(lspv_cont_ids[pos_v4r:lspv_cont_ids.size], lspv_cont_ids[0:pos_v4r])
    pos_v4u = png_which_min2(lspv_ids, v4u)
    pos_v4d = png_which_min2(lspv_ids, v4d)
    ...
    lspv_s1 = lspv_ids[0:pos_v4u]
    lspv_s2 = lspv_ids[pos_v4u: pos_v4d]
    lspv_s3 = lspv_ids[pos_v4d: lspv_ids.size]
...
def get_laa_segments_ids(cont_laa, locator_open, vlaau, vlaad, vlaar):
    ...
    pos_vlaad = png_which_min2(laa_cont_ids, vlaad)
    laa_ids = np.append(laa_cont_ids[pos_vlaad:laa_cont_ids.size], laa_cont_ids[0:pos_vlaad])

    pos_vlaar = png_which_min2(laa_ids, vlaar)
    pos_vlaau = png_which_min2(laa_ids, vlaau)
    ...
    laa_s1 = laa_ids[0:pos_vlaau]
    laa_s2 = laa_ids[pos_vlaau: laa_ids.size]
...
def flat_w_constraints(m, boundary_ids, constraints_ids, x0_b, y0_b, x0_c, y0_c):
    ...
    """
    m = m_open; 
    boundary_ids = seq_contour_ids.astype(int);
    constraints_ids = seq_constraints_ids.astype(int);
    x0_b = x0_bound;
    y0_b = y0_bound;
    x0_c = x0_const;
    y0_c = y0_const
...
def set_piece_label(m, line_textfile, m_seeds):
    # m=m_open; line_textfile; m_seeds
