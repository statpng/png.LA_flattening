...
outfilepath = os.path.join(fileroot, filename[0:len(filename)-4])
...
seeds_filename = outfilepath + '_seeds_for_flat.vtk'
...
to_be_flat_filename = outfilepath + '_to_be_flat.vtk'
m_out = outfilepath + '_flat.vtk'
...
png_remove_nontriangle(m_open)
ExtractVTKTriFaces(m_open)


"""
png_visualize1(m_open)
ExtractVTKPoints(m_open)
ExtractVTKTriFaces(m_open)
png_remove_nontriangle(m_open)
ExtractVTKTriFaces(m_open)
ExtractVTKTriFaces(mesh)
"""
...
# contours
# visualise_default(surfer, surfer, 'STD mesh', 'autolabels', 36, 79)
cont_rspv, cont_ripv, cont_lipv, cont_lspv, cont_mv, cont_laa = extract_LA_contours(m_open, args.meshfile, args.save_conts, edge_length=edge_length)
...
"""
png_visualize1(locator_open.GetDataSet())
png_visualize1(cont_rspv)
png_visualize1(cont_ripv)
"""
...
if args.save_final_paths == True:
    writevtk(path1_clipped_prop, outfilepath + 'path1_prop.vtk')
    writevtk(path2_clipped_prop, outfilepath + 'path2_prop.vtk')
    writevtk(path3_clipped_prop, outfilepath + 'path3_prop.vtk')
    writevtk(path4_clipped_prop, outfilepath + 'path4_prop.vtk')
    writevtk(path5_clipped_prop, outfilepath + 'path5_prop.vtk')
    writevtk(path6_clipped_prop, outfilepath + 'path6_prop.vtk')
    writevtk(path7_clipped_prop, outfilepath + 'path7_prop.vtk')
    writevtk(path_laa1_clipped_prop, outfilepath + 'path_laa1_prop.vtk')
    writevtk(path_laa2_clipped_prop, outfilepath + 'path_laa2_prop.vtk')
...
line_textfile = outfilepath + '_div_lines.txt'
nlines = 9
...
# png_writeCoord(m_final, filename=m_out[:-4]+"_nodes.csv", type="nodes")
# png_writeCoord(m_final, filename=m_out[:-4]+"_edges.csv", type="edges")
