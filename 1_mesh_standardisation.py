...
parser.add_argument('--pv_dist', type=int, default=2, help='PV clipping distance (mm)')
...
parser.add_argument('--save', type=int, default=1, help='Set to 0 to remove intermediate results (centerlines, clippoints, etc.)')
...
path = os.path.dirname(meshfile_open)
################################################
################################################
################################################
...
fileroot = os.path.dirname(meshfile_open) # os.path.dirname(args.meshfile)
...
# writevtk(surface, filename='E:/data/map/flat/5645-outputXML-cut/tmp.vtk')



np.histogram(png_mesh_GetArray2(surface, "zone"))

tmp=surface
# tmp=pointthreshold(surface, "zone", 1, 2)
# png_visualize1(tmp)
writevtk(tmp, args.meshfile[:-4]+"-fill.vtk")

args.meshfile = args.meshfile[:-4]+"-fill.vtk"

png_writestl(args.meshfile, args.meshfile[:-4]+".stl")
surface_reconst(args.meshfile[:-4]+".stl", args.meshfile[:-4]+".stl", depth=10)
writevtk(png_readstl(args.meshfile[:-4]+".stl"), args.meshfile)
...
max_hole_size = 2  # empirical, be careful to do not close pv ostiums, check visually.
stdmesh_closed = fillholes(stdmesh, max_hole_size)
print('\n')
transfer_all_scalar_arrays(surface, stdmesh_closed)

"""
png_mesh_GetArray2(surface,'x')
png_mesh_GetArray2(stdmesh_closed,'x')

png_visualize1(stdmesh_closed)
"""

# convert_cell_data_to_point_data( surface )
# png_mesh_GetArray( convert_cell_data_to_point_data( surface ) )
# writevtk(cleanpolydata(convert_cell_data_to_point_data( surface )), 'E:/data/map/tmp/flat-3218/tmp.vtk')
